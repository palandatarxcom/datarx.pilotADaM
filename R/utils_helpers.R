...
get_header_n <- function(.data, trtp = TRT01P, trtpn = TRT01PN) {
  ...
  .data |>
    ...
    mutate(
      trtp = !!trtp,
      labels = stringr::str_replace_all(stringr::str_wrap(glue::glue('{trtp} (N={N})'), width=10), "\n", function(x) "\\line ")
...
n_pct <- function(n, pct, n_width=3, pct_width=3, digits=0, mark_lt=TRUE) {
  ...
  if (res < .01 & mark_lt) {
    disp <- stringr::str_pad('<1', width=pct_width)
  } else {
  ...
  return(
    ...
    as.character(
      ...
      glue::glue('{format(n, width=n_width)} ({disp}%)')
    )
  )
}
...
sum_subgrp <- function(.data, subgroup_var, order_var = NULL, include.n=TRUE, pad.row=TRUE, header_n = header_n) {
  ...
  subgrps <- .data |>
    ...
    # Transpose
    tidyr::pivot_wider(names_from = TRT01PN, values_from = res) |>
...
desc_stats <- function(.data, var, group = TRT01PN, na.rm=TRUE, int_len=3, size=10, include=c('n', 'Mean', 'SD', 'Median', 'Min', 'Max')) {
  ...
  summaries <- list(
    n      = expr(num_fmt(      n()         , digits=0, int_len=int_len, size=size)),
    Mean   = expr(num_fmt(   mean({{ var }}), digits=1, int_len=int_len, size=size)),
    SD     = expr(num_fmt(     stats::sd({{ var }}), digits=2, int_len=int_len, size=size)),
    Median = expr(num_fmt( stats::median({{ var }}), digits=1, int_len=int_len, size=size)),
    Min    = expr(num_fmt(    min({{ var }}), digits=1, int_len=int_len, size=size)),
    Max    = expr(num_fmt(    max({{ var }}), digits=1, int_len=int_len, size=size))
  ...
  .data |>
    ...
    tidyr::pivot_longer(-{{ group }}, names_to = 'rowlbl2', values_to = 'temp') |>
    # Transpose treatments into separate columns
    tidyr::pivot_wider(names_from = {{ group }}, values_from = temp) |>
    pad_row()
}
...
ae_counts <- function(.data, ..., N_counts = NULL, sort=FALSE) {
  ...
  counts <- subject_counts |>
    ...
    tidyr::pivot_wider(id_cols=c(...), names_from=TRTAN, values_from=c(n, AEs))
...
chi_p <- function(data, results, categories) {
  ...
  p <- stats::chisq.test(res, cats)$p.value
...
fish_p <- function(.data, results, categories, width = 10) {
  ...
  p <- stats::fisher.test(res, cats)$p.value
  if(round(p, 4) == 0) return("<.0001")
  ...
  df <- data |>
    ...
    mutate(
      ...
      mean_sd = as.character(
        glue::glue('{num_fmt(mean, digits=1, int_len=2, size=4)} ({num_fmt(sd, digits=2, int_len=2, size=4)})')
      ),
      med_ran = as.character(
        glue::glue('{num_fmt(median, digits=1, int_len=2, size=4)} ({num_fmt(min, int_len=3, size=3)};{num_fmt(max, int_len=2, size=2)})')
      )
    ) |>
    # Drop the numeric values
    select(-n, -mean, -sd, -median, -min, -max) |>
    # Make summary stats vertical
    tidyr::pivot_longer(c(N, mean_sd, med_ran), names_to = "rowlbl1") |>
    # Split out treatment groups into separate columns
    tidyr::pivot_wider(names_from=TRTPN, values_from=value) |>
    # Fix the row labels
    mutate(rowlbl1 =
             case_when(
               rowlbl1 == 'N' ~ '  n',
...
efficacy_models <- function(data, var=NULL, wk=NULL, model_type='ancova') {
  ...
  if (model_type == 'ancova') {
    if (var == "CHG") {
      model1 <- stats::lm(CHG ~ TRTPN + SITEGR1 + BASE, data=data)
      model2 <- stats::lm(CHG ~ TRTPCD_F + SITEGR1 + BASE, data=data)
    } else {
      model1 <- stats::lm(AVAL ~ TRTPN + SITEGR1, data=data)
      model2 <- stats::lm(AVAL ~ TRTPCD_F + SITEGR1, data=data)
    }
  } else {
  ...
  if (model_type == 'ancova') {
    ...
    sect1 <- tidyr::tibble(rowlbl1=c('p-value(Dose Response) [1][2]'),
  ...
  if (model_type == 'ancova') {
    ...
    sect1 <- tidyr::as_tibble(lsm) |>
      rowwise() |>
      mutate(
        rowlbl1 = 'LS Means (SE)',
        values = as.character(glue::glue('{num_fmt(lsmean, int_len=1, digits=1, size=3)} ({num_fmt(SE, int_len=1, digits=2, size=4)})'))
      ) |>
      select(rowlbl1, TRTPCD_F, values) |>
      tidyr::pivot_wider(id_cols = rowlbl1, names_from=TRTPCD_F, values_from=values) |>
