...
here::i_am("inst/examples/t-14-6-01.R")
library(here)
library(pharmaRTF)
...
#' gen_ht_t14_602
#'
#' Generate Table 14-6.02 Frequency of Normal and Abnormal (Beyond Normal Range) Laboratory Values During Treatment
#'
#' Got error
#'
#' @param adlbc
...
#'
#' @examples
gen_ht_t14_602 <- function(adlbc, adlbh, adsl) {
  ...
  for(i in seq(nrow(adlbc2)/9)) {
    adlbc_pvals[i] <- round(fisher.test(
      matrix(unlist(adlbc2[((i-1)*9+1):(i*9), "N"]),
             nrow = 3,
             ncol = 3,
             byrow = FALSE), workspace=2e9
    )$p.value, 3)
  }

  adlbc3 <- adlbc2 |>
    mutate(n_w_pct = n_pct(N, tot, n_width = 2)) |>
    tidyr::pivot_wider(id_cols = PARAM,names_from = c(TRTP, LBNRIND), values_from = n_w_pct) |>
  ...
  for(i in seq(nrow(adlbh2)/9)) {
    ...
    if(all(mat[,1] == 0) & all(mat[,3] == 0)) {
      ...
      adlbh_pvals[i] <- num_fmt(round(fisher.test(mat, workspace=2e8)$p.value, 3), size = 4, digits = 3, int_len = 1)
    }
  }
...
ht <- gen_ht_t14_602(adam_adlbc, adam_adlbh, adam_adsl)


# Write into doc object and pull titles/footnotes from excel file
doc <- rtf_doc(ht2, header_rows = 3) |>
...
# Write out the RTF
write_rtf(doc, file=here::here("output", "14-6.02.rtf"))
