...
here::i_am("inst/examples/t-14-6-06.R")
library(here)
library(pharmaRTF)
...
#' gen_ht_t14_606
#'
#' Generate Table 14-6.06 Shifts of Hy's Law Values During Treatment
#'
#' @param adlbhy
#' @param adsl
#'
...
#'
#' @examples
gen_ht_t14_606 <- function(adlbhy, adsl) {
    ...
    unlist(lapply(n, function(x) {
      if(x == 0) " 0      "
        ...
        as.character(
          ...
          glue::glue('{format(x, width=n_width)}({format(round((x/pct) * 100), width=pct_width)}%)')
        )
      }
    }))
  }
  ...
  total_tl <- total_t |>
    ...
    tidyr::pivot_wider(id_cols = c("Shift\\line[1]"), names_from = c(TRTP, BASE), values_from = Nc)
  ...
  adlbhy_t <- adlbhy_t1 |>
    ...
    tidyr::pivot_wider(id_cols = c("AVAL"), names_from = c("TRTP", "BASE"), values_from = c("N2"))
...
ht <- gen_ht_t14_606(adam_adlbhy, adam_adsl)
...
write_rtf(doc, file=here::here("output", "14-6.06.rtf"))
